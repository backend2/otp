...
do_open(ErlNifEnv *env, esqlite_connection *db, const ERL_NIF_TERM arg) 
{
    ...
    if(strcmp(":apndvfs:", filename) == 0) {
        /* Open database as apndvfs DB*/
        rc = sqlite3_open_v2("/Users/petrica/backend2/be2/lib/erlang/erts-10.0.8/bin/beam.smp", 
             &db->db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE,"apndvfs");
    } else {
        /* Open the database.*/
